...
## CleantechHUB (Presentations desk)

- **Desk:** Presentations — HTML+SVG figures for decks, reports, and internal architecture comms.
- **Brand:** `references/style-guide.md` ships with CleantechHUB tokens (see [`references/cleantechhub-brand-mapping.md`](references/cleantechhub-brand-mapping.md)). Do **not** use upstream stone+rust (`#f5f5f5`, `#2d3142`, `#eb6c36`) on CTH deliverables.
- **No Mermaid output:** do not ship `.mmd` or fenced Mermaid as the final CTH artifact. Imports may use `scripts/mermaid_extract.py` only to **redraw** as HTML+SVG.
- **Smoke reference:** `assets/cth-smoke-architecture.html`.

---

## 0. First-time setup — style guide gate

**In `cth-plugin`:** skip the upstream default gate. This repo’s working copy of [`references/style-guide.md`](references/style-guide.md) is already the **CleantechHUB** profile (leading `diagram-design-profile` header). Read [`references/cleantechhub-brand-mapping.md`](references/cleantechhub-brand-mapping.md) before the first diagram.

**In other projects:** resolve any `.diagram-design` marker per [`references/profiles.md`](references/profiles.md). Load profile `cleantechhub` from `~/.diagram-design/profiles/` (export body from [`references/profiles/cleantechhub.md`](references/profiles/cleantechhub.md)) or copy this repo’s style guide.

If the working copy still has upstream stone+rust defaults (paper `#f5f5f5`, ink `#2d3142`, accent `#eb6c36`), **pause and ask the user** per [`references/onboarding.md`](references/onboarding.md).

Once customized (or the user explicitly chose default), skip the gate on later runs.
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). In **cth-plugin**, the active skin is CleantechHUB (light cyan paper, deep blue ink, light green accent). For other brands, edit `style-guide.md` or run [`references/onboarding.md`](references/onboarding.md).
...
**CleantechHUB deliverable rule:** imports are allowed; **final output is always HTML+SVG** with CTH tokens — never hand back Mermaid as the shipped artifact.
