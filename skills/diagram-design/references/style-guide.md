...
name: CleantechHUB
slug: cleantechhub
source-url: skills/cleantechhub-brand/brand_dna.yaml
created: 2026-09-24
updated: 2026-09-24
notes: Pre-installed Presentations profile for cth-plugin (Manual de Marca 2024)
-->
...
**CleantechHUB install (`cth-plugin`):** this working copy ships with the CTH five-color palette (not upstream stone+rust). Token mapping from upstream semantic roles is documented in [`cleantechhub-brand-mapping.md`](cleantechhub-brand-mapping.md). To export this skin to `~/.diagram-design/profiles/cleantechhub.md`, copy [`profiles/cleantechhub.md`](profiles/cleantechhub.md).

For non-CTH clients, swap these values or run [`onboarding.md`](onboarding.md).
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#B2EEFA` (light cyan) | `#0C498A` (deep blue) |
| `paper-2` | Diagram container bg, secondary fill | `#9DD4E8` | `#0a3d73` |
| `ink` | Primary text, primary stroke | `#0C498A` (deep blue) | `#FFFFFF` |
| `ink-strong` | High-contrast text on accent fills | `#0C498A` | `#FFFFFF` |
| `muted` | Secondary text, default arrow stroke | `#669348` (forest green) | `#B2EEFA` |
| `soft` | Sublabels, boundary labels | `#4A7A9B` | `#9DD4E8` |
| `rule` | Hairline borders | `rgba(12,73,138,0.12)` | `rgba(178,238,250,0.15)` |
| `rule-solid` | Stronger borders, baselines | `#69B5FA` (sky blue) | `rgba(105,181,250,0.35)` |
| `accent` | Focal / 1–2 max per diagram | `#9DC384` (light green) | `#b5d9a0` |
...
|---|---|---|---|
| `series-1` | `#669348` (forest green) | `#7aad5a` | Non-focal series |
| `series-2` | `#69B5FA` (sky blue) | `#8ec8fc` | Non-focal series |
| `series-3` | `#0C498A` (deep blue) | `#3d7ab8` | Non-focal series |
| `series-4` | `#9DC384` (light green) | `#b5d9a0` | Non-focal series |
| `series-5` | `#4A7A9B` (derived) | `#6a9abb` | Non-focal series |
