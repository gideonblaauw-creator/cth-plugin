...
**Stable HTML host:** `HITL_HTML_STABLE_URL` — LexiScan reference (live): https://fabfloow-ul-playground.vercel.app. **Not** LexiScan product demos (`fabfloow-lexiscan.vercel.app` DEMO frozen, `fabfloow-lexiscan-app.vercel.app` PROD). Other products: Infra plants per-product host; until then Hands leaves the stable-host line as the placeholder token or `[PENDIENTE — Infra plants HITL_HTML_STABLE_URL]`. Do not invent fake URLs. Required for Tier 2; **skip** as a requirement on Tier 1.
