...
| **4 Ship** | Hands auto-loop + Infra | Draft PR + **PR-Agent** + **Playwright/Argos visual CI** on product repos. Hands fix until review loop clears, then **Infra PASS**. Infra PASS **before** step 5. Templates: `docs/examples/software-factory/`. |
...
**Ship CI (t267u):** PR-Agent + Playwright/Argos visual CI. Auto-loop on fail/comments → fix → re-Prove(eng) → Infra PASS. `docs/software-factory-ship-ci.md`.
