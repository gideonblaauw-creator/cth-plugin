...
**Playground format lock (Gideon t1268u):** Tier 1–2 repo playgrounds = **LexiScan house-tour HTML standard** at `understanding-lab/playground/` (`index.html`, `app.js`, `data.js`, `styles.css`, `README.md`, `smoke.sh`). Canonical reference: https://github.com/gideonblaauw-creator/fabfloow-lexiscan (PR #16). FabFloow owns LexiScan product; Infrastructure owns this shared format. Do not invent a third layout per product. Legacy Scanner tabbed microworld shells migrate on separate remediation tickets. Checklist: `docs/understanding-lab-playground-standard.md`. Optional ticket field: `playground_standard: lexiscan-html-v2`.
