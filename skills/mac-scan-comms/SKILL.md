...
name: mac-scan-comms
description: >
  Communications protocol for the Mac Scan Desk. Trigger on Mac Scan Desk comms or Mac hygiene coordination.
...
- **End clients:** Internal — Mac mirror / hygiene only. No external client voice.
...
- **Lane A repo / store for this Desk:** Hands ticket for hygiene scripts; not a jump host (Harness §5).
...
No Workbench. **mac-scan is not a jump host** for VPS or Archive.
...
Do not use mac-scan to reach `/opt/claude-files` or VPS `gws`.

Worker setup and recreation: read `skills/mac-scan-worker/SKILL.md`.
