...
description: >
  Communications protocol for the Sustenttia Desk. Trigger on Sustenttia client comms or Sustenttia Desk coordination.
...
- **End client:** **Sustenttia** (sustainability consulting firm — CTH client). Client contacts: **[PENDIENTE]**.
...
`sustenttia-brand` — client brand managed separately from `cleantechhub-brand`. **No CTH lime leak** into Sustenttia materials (per `skills/app-build/SKILL.md`).
...
- **Lane A repo / store for this Desk:** `https://github.com/gideonblaauw-creator/sustenttia-v2` for app tree; comms packs may be Lane B Drive.
...
Sustenttia trees stay Sustenttia. Private client payloads → local Ollama residency if required.
