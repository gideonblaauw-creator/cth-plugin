...
name: inbox-comms
description: >
  Communications protocol for the Inbox Desk. Trigger on Inbox Desk comms or Gmail coordination.
...
- **End clients:** Email correspondents across CTH and client threads. Named contacts: **[PENDIENTE]** per thread.
...
Follow brand of the owning Desk/thread (`cleantechhub-brand`, `sustenttia-comms` routing, etc.). No cross-client leak.
...
- **Lane A repo / store for this Desk:** Email drafts via Hands (`gemini-3.7-flash`); repo only when ticket says so.
...
**Gmail** Workbench (this Desk only): send when Hands cannot. HITL before send.
...
HITL before every outbound email send.
