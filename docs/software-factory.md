...
| **4** | **Ship** | Hands auto-loop + **Infra PASS** | Draft PR + **PR-Agent** + **Playwright/Argos visual CI** on product repos. Hands fixes until review loop clears, then Infra PASS. Infra PASS = human half of Ship — **before** step 5. |
...
**LexiScan UL public host:** https://fabfloow-ul-playground.vercel.app — Understanding Lab playgrounds only. **Not** LexiScan product demo or production product apps. (Live host cross-link; full plant in PR #31 if not yet merged.)
...
| **Mic Prove (CI)** | **4** (Ship) | Pixel-level UI regression visible to reviewers. | Playwright + Argos CI on the PR. **Supplements** Hands before/after — does not replace pytest/smoke. |
...
- **PR-Agent** ([Codium-ai/pr-agent](https://github.com/Codium-ai/pr-agent)) as GitHub Action on **product repos**: `/describe`, plain-English walkthrough, `/ask` on PR comments.
- **Visual before/after CI:** Playwright (screenshots/video) + Argos CI — posts before/after visual diffs on the PR. AI reviewers do not see pixels; this is the **Mic Prove** surface in CI.
...
- `ARGOS_TOKEN` when using visual CI
...
- Playground format + UL host: `docs/understanding-lab-playground-standard.md` (PR #31)
