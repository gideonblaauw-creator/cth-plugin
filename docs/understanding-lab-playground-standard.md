...
**Ownership:**
- **FabFloow** owns LexiScan **product** content and Notion lab pages for LexiScan builds.
- **Infrastructure** owns this **shared format lock** (repo path, file set, checklist, anti-patterns). Remediation PRs migrate legacy playgrounds toward this standard.
...
| **Path** | `understanding-lab/playground/` on `main` (introduced in PR #16) |
| **Files** | `index.html`, `app.js`, `data.js`, `styles.css`, `README.md`, `smoke.sh` |
| **Notion hub example** | Playground — LexiScan L1 laboral — https://app.notion.com/p/3d9dfee50be981e08a7bfffe2cc2f33f |
| **Ticket field (Tier 1–2)** | `playground_standard: lexiscan-html-v2` (optional but recommended) |

Hands on Tier 1–2 product repos **must** place the repo artifact at `understanding-lab/playground/` (or the product’s agreed Lane A equivalent) using this file set. The Notion Playground page links to the hosted or local artifact; it does not replace the HTML house tour.
...
7. **Product-neutral chrome, product-branded skin** — layout and interaction model match LexiScan; colors/typography follow **that desk’s brand** (FabFloow cream OK for LexiScan). **Never** apply CleantechHUB lime (`#95D5B2`) on non-CTH product playgrounds. **Never** invent a third layout shell.
...
| **2** | Full LexiScan-format house tour; all rooms for the build; stable host when Infra has planted `HITL_HTML_STABLE_URL`. |

Notion Playground URL on the ticket (`lab_playground_url`) points to the **hub page** for that build. The repo path `understanding-lab/playground/` is the **artifact** Infra and reviewers run locally or via stable host.
...
- LexiScan repo: https://github.com/gideonblaauw-creator/fabfloow-lexiscan (`understanding-lab/playground/`, PR #16)
- Example Notion Playground: https://app.notion.com/p/3d9dfee50be981e08a7bfffe2cc2f33f
