...
**One-liner:** Factory step 4 Ship runs **PR-Agent** (plain-English review) plus **visual before/after CI** (Playwright + Argos) on Lane A product repos — not org-wide from `cth-plugin` alone.
...
|---|---|---|
| **PR-Agent** | [Codium-ai/pr-agent](https://github.com/Codium-ai/pr-agent) | `/describe`, plain-English walkthrough, `/ask` on PR comments. AI review loop on the PR. |
| **Visual before/after CI** | Playwright (screenshots/video) + [Argos CI](https://argos-ci.com) | Pixel-level before/after diffs posted on the PR. AI reviewers do not see pixels — this is the **Mic Prove** surface in CI. |
...
| Secret / var | Purpose |
|---|---|
| `OPENAI_API_KEY` | PR-Agent default provider (or equivalent per PR-Agent docs) |
| `GITHUB_TOKEN` | Provided by Actions; PR-Agent uses for PR comments |
| `ARGOS_TOKEN` | Argos CI upload |
...
| Argos project id | Visual baseline project for that product | **[PENDIENTE]** per product |

Document the planted values in the product repo README or Infra runbook — not in this harness repo.

**UL public host** (step 5 playground, not Ship CI): https://fabfloow-ul-playground.vercel.app — planted via PR #31 / harness docs; do not duplicate that plant here.
...
- [ ] Visual CI green when the repo has UI surface (Playwright + Argos job passed or legitimately skipped with ticket note)
...
- UL host (step 5): PR #31 / `docs/understanding-lab-playground-standard.md`
