...
`model` — Cloud Hands only. Allowed: `gemini-3.7-flash` (mechanical/tiny) or `composer-2.5` with **Fast ON** (`fast=true`) for repo/code/builds. Empty build ticket → Composer Fast ON (not Flash). Review/eval/brand with no model → HOLD and flag Gideon. See BUILDING lock t1242u in `docs/hands-model-routing.md` and `skills/harness/SKILL.md` §5.
