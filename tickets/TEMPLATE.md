...
`prove_eng` — optional on `factory: yes`. `pytest+smoke` (default for code builds) or `before-after` when UI/behavior PRs need screenshot/video/metrics in the PR. Often both apply on UI builds.
