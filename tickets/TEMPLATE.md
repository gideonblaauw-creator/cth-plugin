...
`ship_review` — optional; **default `pr-agent` when `factory: yes`**. Step 4 Ship runs **PR-Agent** (Codium-ai/pr-agent: `/describe`, walkthrough, `/ask`) plus **Playwright/Argos visual CI** on the owning product repo. Hands auto-loops on fail/comments until green, then pings Infra PASS. Workflow templates: `docs/examples/software-factory/` (copy into product repo — not org-wide from `cth-plugin`). Reference: `docs/software-factory-ship-ci.md`.

`prove_eng` — optional on `factory: yes`. `pytest+smoke` (default for code builds) or `before-after` when UI/behavior PRs need screenshot/video/metrics in the PR (Hands auto). Often both apply on UI builds. Visual CI **supplements** Hands before/after — does not replace pytest/smoke.
