...
Cloud Hands agents (Cursor cloud agents executing tickets) must read `skills/harness/SKILL.md` before starting any task. That file is the CTH Harness rulebook: five terms (Workbench, Tools, Tickets, Cloud Hands, Desks), ticket lanes, store hierarchy, org, and gates. Ticket contracts live in `tickets/TEMPLATE.md`. Note: `HARNESS.md` in this repo is Cowork/CLI/Cursor setup docs — it is not the CTH Harness rulebook. Token lock **2026-08-26** in §5: Grok Bot coordination only; allowed Cloud Hands models are `gemini-3.7-flash` (mechanical) and `composer-2.5` (`fast=false`) (repo/code); review/eval/brand with no model → HOLD and flag Gideon (no auto-Sonnet). Skill inventory: `skills/harness/references/skill-toolkit.json`. Desk/Hands boundary and Hands-shift lock (2026-08-17), Lane A repo lock (2026-08-19), OSS-first tiny/private (2026-08-20) remain in the same §5. Understanding Lab tiers (Gideon 2026-09-12): default Tier 0; omit `tier:` → treat as Tier 0 and say so; Infra chat and footer lead with the tier label; never surface View Understanding Lab without naming Tier 1 or 2 in the same message; CTA + full five-beat PASS only for named Tier 1–2 (`docs/understanding-lab-tiers.md`).
